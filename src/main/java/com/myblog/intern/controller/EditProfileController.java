...
import com.myblog.intern.request.UserProfileRequest;
...
public class EditProfileController {
    ...
    public ResponseEntity<UserProfileRequest> getUserDetails(HttpServletRequest request) throws ParseException, UnknownHostException {
        String username= jwtService.extractUserName(jwtService.parseToken(request));
        UserProfileRequest userProfileRequest=new UserProfileRequest();
        Integer userIdRequest= userService.getUserIdByUserName(username);
        User user= userService.getUserByName(username);
        UserDetails userDetails=editProfileService.getUserDetailsByUserID(userIdRequest);
        userProfileRequest.setProfilePic(userDetails.getProfilePic());
        userProfileRequest.setBirthDate(userDetails.getBirthDate());
        userProfileRequest.setBirthMonth(userDetails.getBirthMonth());
        userProfileRequest.setBirthYear(userDetails.getBirthYear());
        userProfileRequest.setContact(userDetails.getContact());
        userProfileRequest.setUserName(user.getUserName());
        userProfileRequest.setEmail(user.getEmail());
        userProfileRequest.setFirstName(userDetails.getFirstName());
        userProfileRequest.setLastName(userDetails.getLastName());
        userProfileRequest.setRole(userDetails.getRole());
        userProfileRequest.setCountry(userDetails.getCountry());
        userProfileRequest.setState(userDetails.getState());
        userProfileRequest.setStreet(userDetails.getStreet());
        userProfileRequest.setZipCode(userDetails.getZipCode());

        return new ResponseEntity<> (userProfileRequest, HttpStatus.OK);
    }
